    ...
    buildQuery<T>(
        ...
        for (const queryKey in queryParams) {
            ...
            if (match) {
                ...
                if (typeof filter[fieldName] !== 'object' || Array.isArray(filter[fieldName])) {
                    filter[fieldName] = {};
                }
                ...
                if (!isNaN(parsed) && isFinite(parsed)) {
                    try {
                        filter[fieldName][`$${operator}`] = mongoose.Types.Decimal128.fromString(String(parsed));
                    } catch {
                    ...
                    const trimmed = opValue.trim();
                    if (mongoose.Types.ObjectId.isValid(trimmed)) {
                        filter[fieldName][`$${operator}`] = new mongoose.Types.ObjectId(trimmed);
                    } else {
